...
class TestTab:
    ...
    def _create_mock_ws_client(self):
        """创建模拟 WebSocket 客户端"""
        mock_ws = MagicMock()
    ...
    def test_init(self):
        """测试初始化"""
        mock_ws = self._create_mock_ws_client()
        tab = Tab(tab_id=1, ws_client=mock_ws, url="https://example.com", title="Example")
    ...
    def test_ele_returns_element(self):
        ...
        mock_ws = self._create_mock_ws_client()
        tab = Tab(tab_id=1, ws_client=mock_ws)
...
class TestElement:
    ...
    def _create_mock_tab(self):
        ...
        return Tab(tab_id=1, ws_client=mock_ws)
    ...
    def test_click_sends_click_command(self):
        ...
        tab._ws_client.send_and_receive.assert_called_once()
        call_args = tab._ws_client.send_and_receive.call_args[0][0]
    ...
    def test_text_returns_element_text(self):
        """�试 text 属性返回元素文本"""
        tab = self._create_mock_tab()
        tab._ws_client.send_and_receive.return_value = {
            "success": True,
            "data": "Hello World"
        }
